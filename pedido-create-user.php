...
<head>
  ...
  <title>Novo Pedido - La Vic</title>
  ...
  <script>
    function adicionarProduto() {
      ...
      produtoRow.innerHTML = `
        ...
        <div class="col-md-1 text-center">
          <button type="button" class="btn btn-danger" onclick="this.closest('.row').remove(); atualizarTotal();">X</button>
        </div>
      `;
    ...
    function atualizarTotal() {
      ...
      document.querySelectorAll('.produto-select').forEach((select, i) => {
        ...
        document.querySelectorAll('.subtotal')[i].value =
          subtotal ? 'R$ ' + subtotal.toFixed(2).replace('.', ',') : 'R$ 0,00';
        total += subtotal;
      });
...
<body>
  ...
  <nav class="navbar navbar-expand-lg custom-navbar">
    <div class="container">
      <a class="navbar-brand" href="painel_cliente.php">
  ...
  <div class="container mt-5">
    <h2 class="text-center mb-5">Fazer Novo Pedido</h2>
    <div class="text-center mb-4 text-muted">
      Você está realizando um pedido como <strong><?= htmlspecialchars($nome_cliente) ?></strong>.
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <form action="acoes.php" method="POST">
          ...
          <input type="hidden" name="id_usuario" value="<?= $id_cliente ?>">
          ...
          <div class="text-center">
            <button type="submit" name="create_pedido" class="btn btn-primary px-4">Finalizar Pedido</button>
          </div>
